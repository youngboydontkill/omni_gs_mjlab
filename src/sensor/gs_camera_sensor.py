"""
Module for GS camera sensor, which supports both pinhole and fisheye camera models. 
"""
...
GSCameraType = Literal["pinhole", "fisheye"]
_DEFAULT_CAM_FOVY = 45.0

@dataclass
class GSCameraSensorCfg(SensorCfg):
    ...
    camera_type: GSCameraType = "pinhole"
    """Camera type to capture: "pinhole" or "fisheye"."""
    ...
    def __post_init__(self) -> None:
        valid = {"pinhole", "fisheye"}
        invalid = {dt for dt in self.camera_type if dt not in valid}
        if invalid:
    ...
    def build(self) -> GSCameraSensor:
        return GSCameraSensor(self)

@dataclass
class GSCameraSensorData:
  rgb: torch.Tensor | None = None

class GSCameraSensor(Sensor[GSCameraSensorData]):
  """GS Camera sensor for RGB rendering."""

  requires_sensor_context = True

  def __init__(self, cfg: GSCameraSensorCfg) -> None:
    super().__init__()
    self.cfg = cfg
  ...
  def _compute_data(self) -> GSCameraSensorData:
    # TODO : 这里需要使用额外的render和manager进行管理，避免和mujoco的渲染系统冲突
    ...
    return GSCameraSensorData(rgb=rgb_data)